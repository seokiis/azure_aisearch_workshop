...
| 학습 항목 | 설명 |
|----------|------|
| **📊 Scoring Profile 개념 이해** | 검색 점수에 비즈니스 로직을 적용하여 결과 순위를 조정하는 방법을 배웁니다 |
| **⚖️ 필드 가중치 설정** | 특정 필드(title, brand)에 가중치를 부여해 해당 필드 매칭 시 점수를 높이는 방법을 익힙니다 |
| **📈 Magnitude 함수 활용** | 숫자 필드(가격)를 기준으로 점수를 부스팅하는 방법을 구현합니다 |
| **🏷️ Tag 함수 활용** | 특정 값(프리미엄 브랜드, 주력 카테고리)과 매칭 시 점수를 높이는 방법을 배웁니다 |
| **🔄 프로파일 적용 비교** | 동일 쿼리에 대해 프로파일 적용 전/후 결과를 비교 분석합니다 |
| **💼 비즈니스 시나리오 적용** | 실제 이커머스 검색에서 활용할 수 있는 비즈니스 로직을 구현합니다 |

### 📓 노트북 학습 순서

| 순서 | 노트북 | 핵심 내용 |
|------|--------|----------|
| 1️⃣ | [01-scoring_profile.ipynb](./01-scoring_profile.ipynb) | 기존 인덱스에 Scoring Profile 추가, 필드 가중치, Magnitude/Tag 함수, 5가지 비교 실습 |
...
1. **키워드 검색(BM25)에만 적용**: 벡터 검색 점수에는 영향 없음
2. **하이브리드 검색**: RRF 결합 후 최종 점수에는 영향 없음 (키워드 순위에만 영향)
